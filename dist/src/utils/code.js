...
export async function createVerificationCode(user, type = "email") {
...
export async function verifyCode(user, code, type) {
    const verification = await db.query.verifications.findFirst({
        where: (v, { eq, and }) => and(eq(v.userId, user.id), eq(v.code, code), eq(v.type, type)),
    });
    if (!verification) {
